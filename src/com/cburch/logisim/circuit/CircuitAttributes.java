...
public class CircuitAttributes extends AbstractAttributeSet {
	...
	public <E> void setValue(Attribute<E> attr, E value) {
		if (attr == StdAttr.FACING) {
			...
			facing = val;
			fireAttributeValueChanged(StdAttr.FACING, val, oldFacing);
			if (subcircInstance != null) subcircInstance.recomputeBounds();
		} else if (attr == StdAttr.LABEL) {
			String val = (String) value;
			String oldLabel = label;
			label = val;
			fireAttributeValueChanged(StdAttr.LABEL, val, oldLabel);
		} else if (attr == StdAttr.LABEL_FONT) {
			Font val = (Font) value;
			Font oldFont = labelFont;
			labelFont = val;
			fireAttributeValueChanged(StdAttr.LABEL_FONT, val, oldFont);
		} else if (attr == LABEL_LOCATION_ATTR) {
			Direction val = (Direction) value;
			Direction oldLoc = labelLocation;
			labelLocation = val;
			fireAttributeValueChanged(LABEL_LOCATION_ATTR, val, oldLoc);
		} else {
