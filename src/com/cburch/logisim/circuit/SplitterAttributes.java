...
class SplitterAttributes extends AbstractAttributeSet {
	...
	public <V> void setValue(Attribute<V> attr, V value) {
		...
		if (attr == StdAttr.FACING) {
			...
			oldValue = BitWidth.create(bit_end.length);
			...
			if (val >= 0 && val <= fanout) {
				oldValue = bit_end[bitOutAttr.which];
	...
	private void configureDefaults() {
		...
		for (int i = 0; i < curNum; i++) {
			if (bit_end[i] != dflt[i]) {
				BitOutAttribute attr = (BitOutAttribute) attrs.get(offs + i);
				int oldValue = bit_end[i];
				bit_end[i] = dflt[i];
				fireAttributeValueChanged(attr, Integer.valueOf(bit_end[i]), oldValue);
			}
		}
