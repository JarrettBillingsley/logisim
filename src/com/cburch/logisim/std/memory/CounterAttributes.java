...
class CounterAttributes extends AbstractAttributeSet {
	...
	public <V> void setValue(Attribute<V> attr, V value) {
		...
		if (attr == StdAttr.WIDTH) {
			...
			if (newW > oldW) {
				oldMax = Integer.valueOf(oldWidth.getMask());
				...
				if (v != oldVal) {
					...
					fireAttributeValueChanged(Counter.ATTR_MAX, newValObj, oldValObj);
				}
			}
			fireAttributeValueChanged(StdAttr.WIDTH, newWidth, oldWidth);
		...
		fireAttributeValueChanged(attr, value, value); //??????
		if (newMax != null) {
